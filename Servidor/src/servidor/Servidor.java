...
public class Servidor extends UnicastRemoteObject implements InterfazServidor{
    ...
    @Override
    public synchronized boolean registrarCliente(InterfazCliente cliente, String username, String password,String clavePublica) throws RemoteException{
        if(!this.clientesAuthenticated.authCliente(username, password,cliente,clavePublica)){
    ...
    public synchronized String paso2(String mensaje) throws RemoteException{
        ...
        int nonce = Integer.parseInt(token.nextToken());
        nonce = nonce - 1;
        ...
        mensajedestino = ppal.ejecutarAlgoritmoCifrado(clavedestino, mensajedestino);
